...
(function () {
    ...
    let autoSaveEnabled = true; // cached flag, refreshed on show

    /** Read SRQ_AUTO_SAVE from chrome.storage (feature_flags) */
    async function refreshAutoSaveFlag() {
        try {
            ...
            // Default is true (matches feature_flags.js DEFAULTS)
            autoSaveEnabled = flags.SRQ_AUTO_SAVE !== false;
        } catch { autoSaveEnabled = true; }
    }
    ...
    function createFloatingButton() {
        ...
        // Bind events
        btn.querySelector('.atom-main-icon-container').addEventListener('click', handleMainClick);
    ...
    /** Main icon click: quick-save when auto-save ON, else expand menu */
    ...
    /** One-click save: create SRQ card + toast, skip modal */
    async function quickSave() {
        ...
        try {
            ...
            showToast(getMessage('nlm_quick_saved', 'Saved ✓'), 'success');
        } catch (err) {
            showToast(getMessage('nlm_error_prefix', 'Error: ') + err.message, 'error');
        }
    }
    ...
    function showButton(selection) {
        ...
        refreshAutoSaveFlag(); // non-blocking, updates cached flag
    ...
    async function showPrompt() {
        ...
        try {
            ...
            if (response && response.ok) {
                ...
                try {
                    console.log("[SRQ FB] Sending SRQ_CREATE_CARD...");
                    chrome.runtime.sendMessage({
                        type: "SRQ_CREATE_CARD",
                        payload: {
                            url: window.location.href,
                    ...
                    }).then(r => console.log("[SRQ FB] Response:", r))
                        .catch(e => console.warn("[SRQ FB] Send failed:", e.message));
                    console.log("[SRQ FB] SRQ_CREATE_CARD sent (fire-and-forget)");
                } catch (_srq) { console.warn("[SRQ FB] Sync error:", _srq.message); }
            } else {

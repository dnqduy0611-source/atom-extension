 ...
 * Phase 4: Now delegates to ai/providers/* for API calls
 *
...
(function () {
    ...
    async function getLLMProvider() {
        // Phase 4: Delegate to shared provider module if available
        if (window.AtomProviders?.openrouter?.getConfig) {
            return window.AtomProviders.openrouter.getConfig();
        }
    ...
    function convertToOpenRouterMessages(systemPrompt, geminiContents) {
        // Phase 4: Delegate to shared provider module if available
        if (window.AtomProviders?.openrouter?.convertMessages) {
            return window.AtomProviders.openrouter.convertMessages(systemPrompt, geminiContents);
        }
        // Fallback: inline implementation
        const messages = [];
    ...
    async function callOpenRouterAPI(openrouterKey, model, messages, generationConfig = {}) {
        // Phase 4: Delegate to shared provider module if available
        if (window.AtomProviders?.openrouter?.call) {
            return window.AtomProviders.openrouter.call({

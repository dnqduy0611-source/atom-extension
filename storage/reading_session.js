...
async function getEmbeddingApiKey() {
    try {
        ...
        return data.user_gemini_key || data.gemini_api_key || data.apiKey || '';
    } catch (err) {
        console.warn('[ReadingSession] Failed to read API key:', err);
        return '';
    }
}
...
class ReadingSessionService {
    ...
    static async addHighlight(sessionId, highlight) {
        ...
        if (!isDuplicate) {
            ...
            if (highlight?.text) {
                const checkExisting = window.VectorStore?.getEmbedding
                    ? window.VectorStore.getEmbedding(sessionId)
                    : Promise.resolve(null);
                checkExisting
                    .then((existing) => {
                        if (!existing) {
                            this.embedSession(sessionId).catch(err => {
                                console.warn('[ReadingSession] Auto-embed failed:', err.message);
                            });
                        }
                    })
                    .catch(() => {
    ...
    static async updateCard(sessionId, cardData) {
        ...
        // Auto-embed session after updating card (Phase 3: Semantic Brain)
        if (cardData.keyPoints?.length > 0) {
            this.embedSession(sessionId).catch(err => {
                console.warn('[ReadingSession] Auto-embed failed:', err.message);
            });
        }
    ...
    static async addInsight(sessionId, insightText, source) {
        ...
        // Auto-embed session after adding insight (Phase 3: Semantic Brain)
    ...
    /**
     * Generate and store embedding for a session (Phase 3: Semantic Brain)
     * @param {string} sessionId - Session ID
     * @returns {Promise<boolean>} Success
     */
    static async embedSession(sessionId) {
        // Check if EmbeddingService is available
        if (!window.EmbeddingService || !window.VectorStore) {
            console.log('[ReadingSession] EmbeddingService not available, skipping embedding');
            return false;
        }
        ...
        try {
            ...
            if (!session) {
                console.warn('[ReadingSession] Session not found for embedding:', sessionId);
                return false;
            }
            ...
            if (!apiKey) {
                console.log('[ReadingSession] Missing API key, skipping embedding');
                return false;
            }
            ...
            const rateManager = window.RateLimitManager
                ...
                : null;
            const runEmbedding = () => window.EmbeddingService.embedSession(session, apiKey);
            const embedding = rateManager
                ? await rateManager.enqueue(runEmbedding, { priority: 'background' })
                : await runEmbedding();

            await window.VectorStore.storeEmbedding(session.id, embedding, {
            ...
            console.log('[ReadingSession] Successfully embedded session:', sessionId);
            return true;

        } catch (err) {
            console.error('[ReadingSession] Failed to embed session:', sessionId, err);
            return false;
        }
    }

    /**
     * Get session by ID (alias for getSession for compatibility)
     * @param {string} sessionId - Session ID
     * @returns {Promise<Object|null>} Session or null
     */
...
// Export for use in other files (legacy script context)
if (typeof window !== 'undefined') {
    window.ReadingSessionService = ReadingSessionService;
}

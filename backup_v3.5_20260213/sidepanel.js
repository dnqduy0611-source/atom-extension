    ...
    function renderThreadList() {
            ...
            elements.threadList.innerHTML = visibleThreads.map(thread => {
                ...
                return `
                    <div class="sp-thread-item ${isActive ? 'active' : ''}" data-thread-id="${thread.id}">
                        ...
                        <button class="sp-thread-delete" data-del-id="${thread.id}" title="${getMessage('sp_thread_delete', 'Delete')}" aria-label="Delete">&times;</button>
                    </div>
                `;
        ...
        elements.threadList.querySelectorAll('.sp-thread-item').forEach(item => {
            item.addEventListener('click', (e) => {
                // Don't switch thread when clicking delete button
        ...
        // Add delete handlers
        elements.threadList.querySelectorAll('.sp-thread-delete').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                // If we deleted the active thread, switch to newest remaining
...
Text: "${highlightText.slice(0, promptMaxChars)}"`
    ...
    async function sendToGemini(userMessage, thread, action = null) {
        ...
        try {
            ...
            const systemPrompt = buildSystemPrompt(thread, contextLevel, action, recalledDigests);
     ...
     */
    function buildSystemPrompt(thread, contextLevel = CONTEXT_LEVELS.STANDARD, action = null, recalledDigests = []) {
        const lang = navigator.language.startsWith('vi') ? 'Vietnamese' : 'English';
